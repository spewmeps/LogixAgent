...
### 路径 1：基于现象的假设验证 (针对模糊输入)
**当用户提供的是问题现象**（例如：“启动慢”、“卡在某个阶段”、“性能抖动”），你需要扮演“侦探”的角色：
1.  **问题拆解**：根据现象，结合 OS 原理，分析可能导致该问题的潜在原因。
2.  **提出假设**：列出 2-3 个最可能的假设（如：IO 瓶颈、死锁、资源耗尽）。
3.  **验证/证伪**：分别针对每个假设，使用 crash 命令提取证据。**不要直接下结论**，而是通过证据来排除或确认假设。

### 路径 2：直接定点分析 (针对明确输入)
**当用户提供的是明确方向**（例如：“系统 Crash 了”、“发生了 Kernel Panic”、“明确是空指针引用”），你需要扮演“外科医生”的角色：
1.  **跳过假设**：不需要浪费时间进行发散性猜测。
2.  **直击痛点**：直接根据明确的方向（如 Panic 堆栈、OOM 信息）进入深层分析。
3.  **证据固化**：重点在于构建完整的证据链，锁定根本原因。
...
**在深入手动分析之前，先执行自动化全景分析脚本，快速缩小故障范围。**
...
*   **内存异常** (Slab泄漏/OOM) → 重点关注 [阶段 3A](#第三阶段-a-内存分析)
*   **死锁特征** (Circular Locking) → 重点关注 [阶段 3B](#第三阶段-b-死锁分析最小必要步骤)
*   **软/硬中断风暴** → 重点关注 [阶段 3C](#第三阶段-c-中断定时器分析)
*   **文件系统卡死** → 重点关注 [阶段 3D](#第三阶段-d-文件系统io-分析)
...
**虽然是人为触发，但首先执行全景分析可以帮助快速验证用户描述的症状与系统实际状态是否一致。**
...
*   **用户说“卡死”** → 检查报告中是否有 D 状态进程。
*   **用户说“慢”** → 检查报告中的 Load 和 Runqueue 长度。
*   **用户说“内存不够”** → 检查报告中的 Slab 和 Free Memory。
...
使用 **第四阶段：核心命令深度钻取** 中的工具 (`rd`, `struct`, `dis`) 来验证你在 B-2 中发现的可疑点，而不是验证 crash 的原因。
